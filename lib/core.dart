...
class Core {
      ...
      if (!isConnect) {
        ...
        if (type == 'message') {
          ...
          rowMessage.add({
            ...
            'id': result['id'],
            'name': result['name'],
            'data': base64.decode(result['data']),
            'head color': result['head color'],
            'isRight':false,
            'bubble color': result['bubble color'],
            'size': result['size'],
  ...
  static String sendImage(Uint8List image,String messageId) {
    if (_server == null || _AesKey == null || isConnect==false) {
      return "disconnect";
    }
    try {
      ...
      message['head color'] = headColor;
      message['bubble color'] = bubbleColor;
      message['data'] = base64Encode(image as List<int>);
      String jsonMessage = json.encode(message);
  ...
  static String sendAudio(Uint8List audio,String messageId) {
    if (_server == null || _AesKey == null || isConnect==false) {
      return "disconnect";
    }
    try {
      ...
      message['head color'] = headColor;
      message['bubble color'] = bubbleColor;
      message['data'] =  base64Encode(audio as List<int>);
      String jsonMessage = json.encode(message);
      _server?.write(AesEncrypt(jsonMessage, _AesKey!.split(":")[0], _AesKey!.split(":")[1]));
      return "success";
    } catch (e) {
  ...
  static String sendDisposableImage(Uint8List image,String messageId) {
